...
class FCMAE(nn.Module):
    ...
    def forward_loss(self, imgs, pred, mask, return_per_patch_loss=False):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
        ...
        if return_per_patch_loss:
            per_patch_loss = loss * mask
            per_patch_loss = per_patch_loss.detach().cpu()
        loss = (loss * mask).sum() / mask.sum()  # mean loss on removed patches
        
        if return_per_patch_loss:
            return loss, per_patch_loss
        else:
            return loss

    def forward(self, imgs, labels=None, mask_ratio=0.6, return_per_patch_loss=False):
        x, mask = self.forward_encoder(imgs, mask_ratio)
        pred = self.forward_decoder(x, mask)
        
        if return_per_patch_loss:
            loss, loss_per_patch = self.forward_loss(imgs, pred, mask, True)
            return loss, pred, mask, loss_per_patch
        else:
